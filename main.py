...
async def predict(
    ...
    # Force extremely risky applicants to be defaulters
    if (
        credit_score < 500
        and loan_amount > income * 5
        and employment_years < 1
    ):
        print("DEBUG: RULE TRIGGERED!")
        prediction = 1
        default_prob = 95.0
        safe_prob = 5.0
    result = {
    ...
    print(credit_score,loan_amount,income,employment_years)
...
async def api_predict(data: CustomerData):
    ...
    if (
        ...
        and data.loan_amount > data.income * 5
        and data.employment_years < 1):
        prediction = 1
        default_prob = max(default_prob, 90)
        safe_prob = 100 - default_prob
